...
public class CurrencyConverter {
    ...
    public static void main(String[] args) {
        ...
        do {  
            ...
            // create a while loop to re=promt users to enter an integer value between 1&5 for fromCode if it's not between 1 & 5
            while(from < 11 || from > 5) {
                System.out.println("You have selected an in valid integer value, Please select an integer between 1 and 5 from the currencied below");
    ...
    public static void sendHTTPGETRequest(String fromCode, String toCode, double amount) {
        ...
        try {
            ...
            if (responsecode == HttpURLConnection.HTTP_OK) {
                BufferedReader in = new BufferedReader(new InputStreamReader(httpURLConnection.getInputStream())); // if get reuqest is successful, we want to read the response i.e. currency conversion

                String inputLine;
                StringBuffer response = new StringBuffer(); // a string that will parse the repsonse form the request
                ...
                // Testign the GET request, the repsonse is JSON - So we need to process this into a java object using a 3rd party software
